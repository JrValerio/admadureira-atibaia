...
const destaquesBase = [
  ...
  },
  {
    ...
    icone: (
      <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.6} d="M8 7h8M8 11h8M8 15h5M8 3h6l5 5v11a2 2 0 01-2 2H8a2 2 0 01-2-2V5a2 2 0 012-2z" />
      </svg>
    ),
  },
];
      ...
      <div className="-mx-4 flex gap-4 overflow-x-auto snap-x snap-mandatory px-4 pb-2 md:mx-0 md:grid md:grid-cols-3 md:gap-6 md:overflow-visible md:px-0 md:pb-0 md:snap-none">
        {destaques.map((item) => {
          const cardContent = (
            ...
            >
              <div className="mb-2 md:mb-4 flex justify-center text-brand-orange transition-transform duration-300 group-hover:scale-110">
                {item.icone}
              </div>
              ...
              <span className="mt-3 md:mt-5 inline-block text-xs md:text-sm font-medium text-text-accent-on-dark group-hover:underline">
                {item.label}
              </span>
            </Card>
          );
